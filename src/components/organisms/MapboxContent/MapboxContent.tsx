...
import {
  selectHighlightedPostId,
  selectMapError,
  selectMapFetching,
  selectPostsMap,
  setHighlightedPostId,
} from 'src/store/post/postSlice'
import { useFetchHomesMap } from 'src/store/post/postNetwork'
...
export const PostMarkers = () => {
  useFetchHomesMap()

  const posts = useAppSelector(selectPostsMap).data?.posts || []

  const dispatch = useAppDispatch()

  const markersTransitions = useTransition(posts || [], {
    keys: (post) => post.id,
  ...
  const highlightedHomeId = useAppSelector(selectHighlightedPostId)
    ...
    <>
      {highlightedHomeId === marker.id && (
        <MapPopup
          ...
          onClose={() => dispatch(setHighlightedPostId(null))}
        >
          <PopupHomesContent id={marker.id} />
        </MapPopup>
      )}
